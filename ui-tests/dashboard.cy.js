...
describe('dashboard - UI Components', () => {
  ...
  describe('Modal/Dialog Interactions', () => {
    ...
    it('should render modal "card-header"', () => {
      cy.get('[id*="card-header"]').should('exist');
    });

    it('should be able to close modal "card-header"', () => {
      // Find and interact with close button or backdrop
      cy.get('[id*="card-header"]').within(() => {
        cy.get('button').each($btn => {
          if ($btn.text().includes('Cancel') || $btn.text().includes('Close') || $btn.text().includes('✕')) {
            cy.wrap($btn).should('be.visible');
          }
    ...
    it('should handle keyboard escape for modal "card-header"', () => {
      cy.get('[id*="card-header"]').should('exist');
      cy.get('body').type('{esc}');
      // Verify modal closes or handles escape
    });

    it('should render modal "card kickoff-section"', () => {
      cy.get('[id*="card kickoff-section"]').should('exist');
    });

    it('should be able to close modal "card kickoff-section"', () => {
      // Find and interact with close button or backdrop
      cy.get('[id*="card kickoff-section"]').within(() => {
        cy.get('button').each($btn => {
          if ($btn.text().includes('Cancel') || $btn.text().includes('Close') || $btn.text().includes('✕')) {
            cy.wrap($btn).should('be.visible');
          }
    ...
    it('should handle keyboard escape for modal "card kickoff-section"', () => {
      cy.get('[id*="card kickoff-section"]').should('exist');
      cy.get('body').type('{esc}');
      // Verify modal closes or handles escape
    });
