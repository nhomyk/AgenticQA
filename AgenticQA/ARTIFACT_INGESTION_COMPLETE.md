...
✅ Hybrid RAG mode available (NEW) - Vector + Relational storage

## 🔀 Hybrid Storage (NEW!)

### Two-Database Architecture

AgenticQA now supports **Hybrid RAG** combining:
- **Vector DB (Weaviate):** Unstructured artifacts (test logs, error messages, patterns)
- **Relational DB (SQLite/PostgreSQL):** Structured metrics (coverage %, pass rates, counts)

### Why Hybrid?

| Benefit | Impact |
|---------|--------|
| **Cost Savings** | 70-98% reduction for structured queries |
| **Performance** | 10-40x faster for metric queries |
| **Resilience** | Fallback when Weaviate unavailable |
| **Zero Config** | SQLite works out of the box |

### Enable Hybrid Mode

```bash
# Add to GitHub Secrets or CI environment
export AGENTICQA_HYBRID_RAG=true
```

All ingestion continues to work automatically - data is smartly routed to the appropriate store!

See [HYBRID_RAG_ARCHITECTURE.md](./HYBRID_RAG_ARCHITECTURE.md) for details.
...
**NEW:** Hybrid storage option for optimal cost and performance!

---

**Status:** ✅ COMPLETE - Full artifact ingestion with SRE agent pipeline failure learning + Hybrid storage
**Next:** Monitor agent improvements over time via Weaviate dashboard or enable Hybrid RAG mode for cost savings
