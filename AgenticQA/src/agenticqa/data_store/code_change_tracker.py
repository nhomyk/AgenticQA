...
class CodeChangeTracker:
    ...
    def _analyze_impact(
        ...
        tests_improved = after_tests_pass > before_tests_pass or after_tests_fail < before_tests_fail
        ...
        safe_to_deploy, reason = self._determine_deployment_safety(
            quality_improved, performance_improved, tests_improved, 
            compliance_improved, quality_delta, after_tests_fail
        )
    ...
    def _determine_deployment_safety(
        ...
        if quality_delta < -10:
            return False, f"Quality degraded significantly ({quality_delta:.1f}%) - regression detected"
...
class ChangeImpactReport:
    ...
    def generate_report(metrics: BeforeAfterMetrics) -> str:
        ...
        report.append(f"  Before: {metrics.before_tests_passed} passed, {metrics.before_tests_failed} failed")
        report.append(f"  After:  {metrics.after_tests_passed} passed, {metrics.after_tests_failed} failed")
