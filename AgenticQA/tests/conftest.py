...
def mock_qa_agent():
    ...
    agent = MagicMock()
    agent.decide = Mock(return_value={
        'agent': 'QA',
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "unit: mark test as unit test (isolated, no dependencies)"
    )
    config.addinivalue_line(
    ...
    config.addinivalue_line(
        "markers", "deployment: mark test as deployment readiness test"
    )
    config.addinivalue_line(
