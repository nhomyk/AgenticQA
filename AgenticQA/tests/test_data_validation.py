...
def sample_dataset():
    ...
    return pd.DataFrame({
        ...
        'email': ['alice@example.com', 'bob@example.com', 'charlie@example.com', 
                  'alice2@example.com', 'david@example.com'],
        'age': [25, 30, 35, 26, 28],
...
class TestDataIntegrity:
    ...
    def test_row_count_preserved(self, sample_dataset, modified_dataset):
        ...
        assert len(sample_dataset) == len(modified_dataset), \
    ...
    def test_data_types_not_corrupted(self, sample_dataset):
        ...
        assert pd.to_numeric(sample_dataset['age'], errors='coerce').notna().all(), \
            "Age column contains non-numeric values"
        
        # Salary should be numeric
        assert pd.to_numeric(sample_dataset['salary'], errors='coerce').notna().all(), \
...
class TestDataQuality:
    ...
    def test_no_null_values_in_critical_fields(self, sample_dataset):
        ...
        for field in critical_fields:
            assert sample_dataset[field].notna().all(), \
    ...
    def test_no_future_dates(self, sample_dataset):
        ...
        assert (sample_dataset['created_at'] <= datetime.now()).all(), \
...
class TestDuplicateDetection:
    ...
    def test_email_similarity_detection(self, sample_dataset):
        ...
        assert any('alice' in pair['email1'] and 'alice' in pair['email2'] 
                  for pair in similar_pairs), "Similar alice emails not detected"
    ...
    def test_encryption_validation(self, pii_dataset):
        ...
        for field in sensitive_fields:
            if field in pii_dataset.columns:
                assert pii_dataset[field].notna().all(), \
